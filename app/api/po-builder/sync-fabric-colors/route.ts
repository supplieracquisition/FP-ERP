...
import { pobFabricColors, apiKeys } from "@/lib/db/schema";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Delete all existing fabric colors
    await db.delete(pobFabricColors);

    // Insert new data
    if (data.length > 0) {
      await db.insert(pobFabricColors).values(
        data.map((item: any) => ({
    ...
    return NextResponse.json({
      ok: true,
      message: "Fabric colors synced",
      imported: data.length,
      updated: 0,
      errors: 0,
    });
  } catch (error: any) {
    console.error("Error syncing fabric colors:", error);
    return NextResponse.json(

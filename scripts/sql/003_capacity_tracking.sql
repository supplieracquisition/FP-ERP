...
--
-- Adds suppliers.weekly_capacity (orders per week) and order_items.assigned_at
...
-- ---------------------------------------------------------------------------
-- Expected: weekly_capacity and assigned_at both ABSENT; capacity_units and
...
WHERE (table_name = 'suppliers'   AND column_name IN ('weekly_capacity','capacity_units','production_time'))
   OR (table_name = 'order_items' AND column_name IN ('assigned_at','claimed_at','supplier_id'))
ORDER BY table_name, column_name;

-- How much of the backfill will land. assigned_at is reconstructed from
...
ALTER TABLE order_items ADD COLUMN IF NOT EXISTS assigned_at TEXT;

-- Intake is a trailing-window count, so it is read far more often than it is
-- written and always alongside supplier_id.
CREATE INDEX IF NOT EXISTS idx_order_items_assigned_at
  ON order_items (supplier_id, assigned_at);
...
WHERE (table_name = 'suppliers'   AND column_name = 'weekly_capacity')
   OR (table_name = 'order_items' AND column_name = 'assigned_at')
ORDER BY table_name;

SELECT
  count(*) FILTER (WHERE assigned_at IS NOT NULL)                            AS backfilled,
  count(*) FILTER (WHERE supplier_id IS NOT NULL AND assigned_at IS NULL)    AS assigned_but_untimed,
  -- Must be 0: an unassigned order can never have an assignment time.
  count(*) FILTER (WHERE supplier_id IS NULL AND assigned_at IS NOT NULL)    AS phantom_intake
FROM order_items;

SELECT indexname FROM pg_indexes
WHERE tablename = 'order_items' AND indexname = 'idx_order_items_assigned_at';
